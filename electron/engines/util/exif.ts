...
function toMetadata(tags: Record<string, { description: string } | undefined>): ImageMetadata {
  ...
  if (tags.PixelXDimension?.description) meta.width = Number(tags.PixelXDimension.description)
  if (tags.PixelYDimension?.description) meta.height = Number(tags.PixelYDimension.description)
  return meta
}

export async function readImageMetadata(bytes: Uint8Array): Promise<ImageMetadata> {
  const ExifReader = (await import('exifreader')).default
  const tags = ExifReader.load(bytes.buffer, { expanded: false }) as Record<
    string,
